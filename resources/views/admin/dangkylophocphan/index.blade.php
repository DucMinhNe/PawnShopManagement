        ...
        $(function() {
                ...
                if ($selectElement == "id_mon_hoc") {
                    $data.mon_hoc.forEach(element => {
                        if ($checkedvalue.length > 0 && $checkedvalue[0] == element.id)
                            $textHTML += '<option value="' + element.id + '" selected>' + element
                            .ten_mon_hoc +
                            '</option>'
                        else
                ...
                if ($selectElement == "id_lop_hoc_phan") {
                    $data.danh_sach_lop_hoc_phan.forEach(element => {
                        if ($checkedvalue.length > 0 && $checkedvalue[0] == element.lop_hoc_phan.id)
                            $textHTML += '<option value="' + element.lop_hoc_phan.id + '" selected>' +
                            element
                            .lop_hoc.ten_lop_hoc + ' - ' + element.lop_hoc_phan.ten_lop_hoc_phan +
                            '</option>'
                        else
                            $textHTML += '<option value="' + element.lop_hoc_phan.id + '">' + element
                            .lop_hoc
                    ...
                    if ($data.status == 1) {
                        $data.sinh_vien.forEach(element => {
                            if ($checkedvalue.length > 0 && $checkedvalue[0] == element.ma_sv)
                                $textHTML += '<option value="' + element.ma_sv + '" selected>' + element
                                .ma_sv +
            ...
            $('body').on('click', '.editBtn', function() {
                ...
                $.get("{{ route('dangkylophocphan.index') }}" + '/' + id + '/edit', function(data) {
                    ...
                    $url =
                    "{{ env('SERVER_URL') }}/api/sinhvien/danhsachsinhvientheokhoatheonganh";
